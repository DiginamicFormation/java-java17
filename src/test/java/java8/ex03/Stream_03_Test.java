...
public class Stream_03_Test {
	...
	public void test_flatMap() throws Exception {
		...
		List<Pizza> result = orders.stream().map(o -> o.getPizzas()).flatMap(p -> p.stream()).collect(Collectors.toList());
	...
	public void test_flatMap_distinct() throws Exception {
		...
		List<Pizza> result = orders.stream().map(o -> o.getPizzas()).flatMap(p -> p.stream()).distinct().collect(Collectors.toList());
